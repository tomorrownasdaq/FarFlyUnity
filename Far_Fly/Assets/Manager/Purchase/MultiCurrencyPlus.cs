...
public class MultiCurrencyPlus : MonoBehaviour, IStoreListener
{
    [SerializeField]
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        foreach (var group in currencyGroups)
        {
            foreach (var item in group.items)
            {
                ...
                {
                    int quantity = GetPurchasedQuantity(args);
                    int amount = GetCurrencyAmount(item) * quantity;
    ...
    private int GetPurchasedQuantity(PurchaseEventArgs args)
    {
        // 구글 플레이에서 제공하는 수량 정보 추출
        var googlePlayReceipt = GooglePurchase.FromJson(args.purchasedProduct.receipt);
        if (googlePlayReceipt != null && (GooglePurchaseState)googlePlayReceipt.purchaseState == GooglePurchaseState.Purchased)
        {
            return googlePlayReceipt.quantity;
        }
        return 1; // 기본값
    }
...
// 구글 플레이 구매 정보를 파싱하기 위한 helper 클래스
[System.Serializable]
public class GooglePurchase
{
    ...
    public static GooglePurchase FromJson(string jsonString)
    {
        var purchaseInfo = JsonUtility.FromJson<GooglePurchase>(jsonString);
        return purchaseInfo;
    }
}
